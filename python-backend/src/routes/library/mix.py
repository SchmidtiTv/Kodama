...
def update_playlist_mix(playlist_id: str) -> RouteResponse:
    ...
    try:
        return jsonify(playlist_mix().update(music_session().state.current_profile, playlist_id, data))
...
def start_playlist_mix_analysis(playlist_id: str) -> RouteResponse:
    ...
    for track in tracks:
        ...
        if not all(isinstance(value, str) and value.strip() and len(value) <= 256 for value in (instance_id, video_id)):
            return jsonify({"error": "tracks require instanceId and videoId"}), 400
        normalized.append({"instanceId": instance_id, "videoId": video_id})
    return jsonify(mix_analysis_service().start(music_session().state.current_profile, playlist_id, normalized)), 202
