...
export function BugReportModal({ onClose, screenshot, t, version, currentTrack }) {
  ...
  useEffect(() => {
    let cancelled = false;
    fetch(`${API}/diag`).then(r => r.json()).then(d => { if (!cancelled) setDiag(d); }).catch(() => {});
  ...
  const submit = async () => {
    ...
    try {
      const r = await fetch(`${API}/feedback`, {
        ...
        body: JSON.stringify({
          title: title.trim(), category, area, severity,
          description: description.trim(), steps: steps.trim(), expected: expected.trim(),
