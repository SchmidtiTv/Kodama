...
function FaIcon({ name, size = 14 }) {
  return (
    <i
      ...
      style={{ fontSize: size, width: size, textAlign: "center", flexShrink: 0, lineHeight: 1 }}
    />
  );
}
...
function buildCornerPathData(W, H, corners, ox = 0, oy = 0) {
  ...
  if (br.t === "r")
    d += `L ${ox + W} ${oy + H - br.s} Q ${ox + W} ${oy + H} ${ox + W - br.s} ${oy + H} `;
...
function CornerInputMixed({ corner, type, value, onChangeType, onChangeValue, min = 0, max = 60 }) {
  ...
  return (
    ...
    >
      {/* Round button */}
      <button
        onClick={() => onChangeType(corner, "r")}
        onMouseEnter={(e) => {
          if (type !== "r") e.currentTarget.style.background = "var(--surface-2)";
        }}
        onMouseLeave={(e) => {
          if (type !== "r") e.currentTarget.style.background = "transparent";
        }}
        style={{
          display: "flex",
          alignItems: "center",
          ...
          flexShrink: 0,
          background:
            type === "r" ? "color-mix(in srgb, var(--accent) 22%, transparent)" : "transparent",
      ...
      {/* Bevel button */}
      <button
        onClick={() => onChangeType(corner, "b")}
        onMouseEnter={(e) => {
          if (type !== "b") e.currentTarget.style.background = "var(--surface-2)";
        }}
        onMouseLeave={(e) => {
          if (type !== "b") e.currentTarget.style.background = "transparent";
        }}
        style={{
          display: "flex",
          alignItems: "center",
          ...
          flexShrink: 0,
          background:
            type === "b" ? "color-mix(in srgb, var(--accent) 22%, transparent)" : "transparent",
...
function lockSettingsSection(ms = 600) {
...
function subscribeSettingsSection(l) {
  ...
  return (
    <div style={{ paddingBottom: 32 }}>
      ...
      >
        {artist.thumbnail ? (
          <img
            src={thumb(hiResThumb(artist.thumbnail))}
            alt=""
            style={{
              position: "absolute",
              inset: 0,
            ...
            so it shows through the transparent sidebar/canvas while cards keep their own bg). */}
              ...
              card and everything inside it stay clickable (they're children, not the region). */}
                        ...
                        >
                          ...
                          >
                            ...
                            >
                              ...
                              {translate(
                                language,
                                selectedTracks.size === 1 ? "songSelected" : "songsSelected"
                              )}
                            </div>
                            {/* Actions */}
                            <div
                                  ...
                                  <SelActionBtn
                                    ...
                                    onClick={async () => {
                                      ...
                                      for (const track of tracks) {
                                        ...
                                        try {
                                          await fetch(
                                            `${API}/playlist/${collection.playlistId}/remove`,
                                            {
                                              method: "POST",
                                              headers: { "Content-Type": "application/json" },
                                              body: JSON.stringify({
                  ...
                  {downloadBatches.length > 0 &&
                    (() => {
                      const overallDone = downloadBatches.reduce(
                        (s, b) => s + b.completedCount + b.errorCount,
                        0
                      );
                      const overallTotal = downloadBatches.reduce(
                        (s, b) => s + b.videoIds.length,
                        0
                      );
                        ...
                        >
                          <div className="flex items-center gap-2">
                            ...
                            <Button
                              ...
                              size="sm"
                              isIconOnly
                              onPress={() => setDownloadQueueMin((m) => !m)}
                              aria-label={downloadQueueMin ? "Expand" : "Minimize"}
                            >
                          ...
                          {!downloadQueueMin &&
                            downloadBatches.map((batch) => {
                              ...
                              const pct = total
                                ? Math.round((batch.completedCount / total) * 100)
                                : 0;
                              return (
                                <div key={batch.id} className="flex items-center gap-3">
                                  ...
                                  <div className="flex-1 min-w-0">
                                    <div className="flex items-center gap-2">
                                      ...
                                      <div className="text-t12 font-semibold truncate flex-1">
                                        {batch.title}
                                      </div>
                                      {!isFinished && (
                                        <Button
                                          ...
                                          onPress={() => handleCancelBatch(batch.id)}
