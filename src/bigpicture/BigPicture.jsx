...
const TAB_SCREEN = { home: "home", search: "search", playlists: "browse-playlists", albums: "browse-albums", artists: "browse-artists" };
const tabOfScreen = (s) => (s === "home" || s === "search") ? s : (s.startsWith("browse-") ? s.slice(7) : null);
...
export function BigPicture() {
  ...
  const menuIndexRef = useRef(menuIndex); menuIndexRef.current = menuIndex;
  const openRef = useRef(open); openRef.current = open;

  const openMenu = useCallback(() => {
    const target = getContextTarget();
    if (target && target.actions && target.actions.length) { setMenuIndex(0); setMenu(target); playOpen(); }
  }, []);
  ...
  const switchTab = useCallback((key) => { histRef.current = []; setScreen(TAB_SCREEN[key]); }, []);
  ...
  useEffect(() => {
    ...
    const onKey = (e) => {
      ...
      if (e.key === "ArrowUp" || e.key === "ArrowDown" || e.key === "ArrowLeft" || e.key === "ArrowRight") playNav();
  ...
  useEffect(() => {
    if (open && !menu) { resume(); return; }
    pause();
    if (!open) { setScreen("home"); histRef.current = []; setMenu(null); }
  }, [open, menu]);

  // Drop any stale context-menu target when the screen changes.
  useEffect(() => { clearContextTarget(); }, [screen]);

  // Keyboard control of the context menu (norigin is paused while it's open).
  useEffect(() => {
    if (!menu) return;
    const onKey = (e) => {
      if (e.key === "ArrowUp") { e.preventDefault(); playNav(); setMenuIndex(i => Math.max(0, i - 1)); }
      else if (e.key === "ArrowDown") { e.preventDefault(); playNav(); setMenuIndex(i => Math.min(menu.actions.length - 1, i + 1)); }
      else if (e.key === "Enter") { e.preventDefault(); playSelect(); const a = menu.actions[menuIndexRef.current]; setMenu(null); a && a.run(); }
      else if (e.key === "Escape" || e.key === "Backspace") { e.preventDefault(); playBack(); setMenu(null); }
    };
  ...
  useEffect(() => {
    ...
    const onKey = (e) => { if (!e.bpSynthetic) setInputMode("key"); };
    window.addEventListener("keydown", onKey, true);
    return () => window.removeEventListener("keydown", onKey, true);
  }, [open]);
  ...
  const onDirection = useCallback((d) => {
    ...
    if (menuRef.current) {
      if (d === "up") { playNav(); setMenuIndex(i => Math.max(0, i - 1)); }
      else if (d === "down") { playNav(); setMenuIndex(i => Math.min(menuRef.current.actions.length - 1, i + 1)); }
      return;
    }
  ...
  const onEnter = useCallback(() => {
    setInputMode("pad");
    if (menuRef.current) { playSelect(); const a = menuRef.current.actions[menuIndexRef.current]; setMenu(null); a && a.run(); return; }
    synthKey("Enter", 13);
  }, []);
  // Navigate forward, remembering where we came from so B pops back through the trail.
  const go = useCallback((next) => { histRef.current.push(screenRef.current); setScreen(next); }, []);
  const onBack = useCallback(() => {
    setInputMode("pad");
    playBack();
    if (menuRef.current) { setMenu(null); return; } // B first dismisses the menu
    const prev = histRef.current.pop();
    if (prev != null) setScreen(prev); else setOpen(false);
  }, []);
  ...
  const openDetail = useCallback((type, item) => {
    ...
    else if (type === "artists") { setArtistItem(item); go("artist"); }
  }, [go]);
  // From the artist screen, opening one of its albums reuses the album detail screen.
  const openAlbumFromArtist = useCallback((album) => { setDetailItem({ type: "albums", item: album }); go("detail"); }, [go]);

  if (!open) return null;

  // Shared top chrome (icon tabs + now-playing card) for the tab-level screens only.
  const activeTab = tabOfScreen(screen);
  const chrome = activeTab ? <TabChrome active={activeTab} onSelectTab={switchTab} onOpenNowPlaying={() => go("nowplaying")} /> : null;
  ...
  else if (screen === "detail" && detailItem) content = <Detail type={detailItem.type} item={detailItem.item} onPlayed={() => go("nowplaying")} />;
  else if (screen === "artist" && artistItem) content = <Artist item={artistItem} onOpenAlbum={openAlbumFromArtist} onPlayed={() => go("nowplaying")} />;
  else if (screen.startsWith("browse-")) content = <Browse type={screen.slice(7)} chrome={chrome} onSelect={(it) => openDetail(screen.slice(7), it)} />;
  ...
  return (
    <FocusContext.Provider value={focusKey}>
      ...
      <Keybar hints={hints} />
      {menu && (
        <div style={{ position: "fixed", inset: 0, zIndex: 2147483100, display: "flex", alignItems: "center", justifyContent: "center", background: "rgba(0,0,0,0.55)" }}>
          <div style={{ width: 460, maxWidth: "80vw", background: "#15101c", borderRadius: 18, padding: 10, boxShadow: "0 30px 80px rgba(0,0,0,.6)", border: "1px solid rgba(255,255,255,0.08)" }}>
